...
# Parsing methods
    ...
    def parse_trade(self, trade, market=None):
        ...
        if timestamp is not None:
            if timestamp < 10 ** 12:  # Check if it's in seconds
    ...
    def parse_funding_rate(self, fundingRate, market=None):
        ...
        timestamp = self.safe_integer(latest, 'time')
        if timestamp is not None:
            if timestamp < 10 ** 12:  # Check if it's in seconds
                timestamp *= 1000  # Convert to milliseconds
        return {
    ...
    def parse_ohlcv(self, ohlcv, market= None) -> list:
        if isinstance(ohlcv, list):
            timestamp = self.safe_integer(ohlcv, 0)  # Original timestamp extraction
            # Convert from seconds to milliseconds if necessary
            if timestamp and timestamp < 10 ** 12:  # Check if it's in seconds
                timestamp *= 1000
            return [
    ...
    def parse_position(self, position, market=None):
        ...
        return {
            ...
            'time': timestamp,
            'isolatedMargin': None,
            'marginType': 'isolated' if self.safe_integer(position, 'marginType') == 92 else 'cross',
            'positionSide': self.safe_string(position, 'positionDirection'),
            'takeProfitOrder': self.safe_value(position, 'takeProfitOrder'),
            'stopLossOrder': self.safe_value(position, 'stopLossOrder'),
